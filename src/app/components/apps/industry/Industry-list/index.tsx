...
function IndustryList() {
  const { industries, deleteIndustry } = useContext(IndustryContext);
  ...
  const Shipped = industries.filter(
    (t: { status: string }) => t.status === "Shipped"
  ).length;
  const Delivered = industries.filter(
    (t: { status: string }) => t.status === "Delivered"
  ).length;
  const Pending = industries.filter(
  ...
  const toggleSelectAll = () => {
    ...
    if (selectAllValue) {
      setSelectedProducts(industries.map((industry: { id: any }) => industry.id));
    } else {
      setSelectedProducts([]);
    }
  };
            ...
            {filteredIndustries.map(
              (industry: {
                ...
                <TableRow key={industry.id}>
                  <TableCell padding="checkbox">
                    <CustomCheckbox
                      checked={selectedProducts.includes(industry.id)}
                      onChange={() => toggleSelectProduct(industry.id)}
                    />
                  </TableCell>
                  <TableCell>
                    <Typography variant="h6" fontSize="14px">
                  ...
                  <TableCell align="center">
                    ...
                    <Tooltip title="Delete Industry">
                      <IconButton
                        color="error"
                        onClick={() => {
                          setSelectedProducts([industry.id]);
                          handleDelete();
                        }}
                      >
                        <IconTrash width={22} />
