...
const companyBaseSchemaUpdate = z.object({
  minQty: commonNumber.optional().refine(val => !isNaN(Number(val)), { message: "Minimum Quantity must be a valid number" }).optional(),
  ticker: z.string().max(10, "Ticker must be at most 10 characters").nonempty("Ticker is required").optional(),
  slug: z.string().max(100, "Slug must be at most 100 characters").nonempty("Slug is required").optional(),
  name: z.string().max(100, "Name must be at most 100 characters").nonempty("Name is required").optional(),
  status: z.boolean().default(true).optional(),
  maxQty: commonNumber.optional().refine(val => !isNaN(Number(val)), { message: "Maximum Quantity must be a valid number" }),
  ...
  seoHeader: z.string().optional(),
  seoContent: z.string().optional(),
  keywords: z.array(z.string()).optional(),
  ipoPrice: commonString.optional(),
  ipoDate: periodString.optional(),
  preIpoDate: periodString.optional(),
  qty: commonNumber.refine(val => !isNaN(Number(val)), { message: "Quantity must be a valid number" }).optional(),
  sectorId: objectIdSchema.optional(),
  dhrpId: objectIdSchema.optional(),
  depositsId: z.array(objectIdSchema).min(1, "At least one deposit is required").optional(),
  performanceId: objectIdSchema.optional(),
  isin: z.string().max(50, "ISIN must be at most 50 characters").optional(),
  pan: z.string().max(50, "PAN must be at most 50 characters").optional(),
  location: z.string().max(200, "Location must be at most 200 characters").optional(),
  rating: commonNumber.optional().default("0").optional(),
  price: commonNumber.refine(val => !isNaN(Number(val)), { message: "Price must be a valid number" }),
  lot: commonNumber.optional().refine(val => !isNaN(Number(val)), { message: "Lot must be a valid number" }),
  email: z.string().email("Invalid email format").max(50, "Email must be at most 50 characters"),
  ...
  shareholderFile: z.string().max(100, "Shareholder file URL must be at most 100 characters").optional(),
  dhrpLink: z.string().optional(),
  management: z.array(createManagementDto).min(1, "At least one management member is required").optional(),
  aboutus: z.string().min(1, "About Us is required").optional(),
  faq: z.array(faqDto).min(1, "At least one FAQ is required").optional(),
  financialResults: z.array(financialResultsSchema).optional(),
  shareHolders: z.array(createShareholderDto).min(1, "At least one shareholder is required").optional(),
});
